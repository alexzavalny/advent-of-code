...
def solution(input)
  ...
  10000.times do
    ...
    puts "Chamber size = #{@chamber.size}. Previous chamber size = #{chamber_size}"
    @increases << @chamber.size - chamber_size - 1
    chamber_size = @chamber.size - 1
  ...
  @current_rock = nil
  print_chamber_with_rock
  File.open("pattern0.txt", "w") { |f| f.write @increases.join(",") }
end

require 'benchmark'
puts Benchmark.measure { solution(File.readlines("input0.txt").first) }